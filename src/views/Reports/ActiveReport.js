...
function ActiveReport() {
  ...
  const reportColumns = [
    ...
    { accessorKey: 'parent', header: 'Parent', size: 80 },
    { accessorKey: 'type', header: 'Type', size: 80 },
    { accessorKey: 'direction', header: 'Direction', size: 80 },
    { accessorKey: 'venue', header: 'Venue', size: 80 },
    {
      ...
      try {
        ...
        if (selectedSections.date) {
          response = await apiCalls(
            'get',
            `/activities/getActiveReport?finYear=${finYear}&orgId=${orgId}&clientName=${encodeURIComponent(formData.clientName)}&fromDate=${formData.fromDate}&toDate=${formData.toDate}&branchName=${formData.branch}`
          );
        } else {
          response = await apiCalls(
            'get',
            `/activities/getActiveReport?finYear=${finYear}&orgId=${orgId}&clientName=${encodeURIComponent(formData.clientName)}&branchName=${formData.branch}`
          );
        }
        if (response.status === true) {
          console.log('Response:', response);
          setRowData(response.paramObjectsMap.activeReportDeatils);
          setIsLoading(false);
          setListView(true);
        } else {
  ...
  // pdf download
  const handleDownloadPdf = ({ logo, columns, data, fileName, loginUserName, formData, selectedSections }) => {
    ...
    const columnStyles = {
      ...
      8: { cellWidth: 25 }, // assn name
      9: { cellWidth: 25 }, // assn to
  ...
  const handleDownloadExcel = async ({ logo }) => {
    try {
      ...
      const sheet = workbook.addWorksheet('Active Report');
      sheet.state = 'visible';
      ...
      titleCell.value = 'Active Report';
      titleCell.font = { size: 18, bold: true, color: { argb: 'FF34449B' } };
      titleCell.alignment = { horizontal: 'center', vertical: 'middle' };
      const metadata = [];
      // if (formData.fromDate && dayjs(formData.fromDate).isValid()) {
      if (selectedSections.date) {
      ...
      saveAs(blob, `Active_Report_${dayjs().format('YYYYMMDD_HHmmss')}.xlsx`);
    } catch (error) {
