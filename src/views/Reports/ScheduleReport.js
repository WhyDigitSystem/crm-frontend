...
import { getAllScheduleBranches } from 'utils/CommonFunctions';
...
function ScheduleReport() {
  ...
  const reportColumns = [
    ...
    { accessorKey: 'customerName', header: 'Customer Name', size: 80 },
    { accessorKey: 'priority', header: 'Priority', size: 80 },
    { accessorKey: 'taskName', header: 'Task Name', size: 80 },
    {
      accessorKey: 'startTime',
      header: 'Start Time',
      size: 50,
      Cell: ({ cell }) => <div style={{ textAlign: 'center', width: '100%' }}>{cell.getValue() || '-'}</div>
    },
    {
      accessorKey: 'endTime',
      header: 'End Time',
    ...
    { accessorKey: 'assiggnedName', header: 'Assn Name', size: 80 },
    { accessorKey: 'assignedTo', header: 'Assn To', size: 80 },
      ...
      try {
        ...
        if (selectedSections.date) {
          response = await apiCalls(
            'get',
            `/activities/getScheduleReport?finYear=${finYear}&orgId=${orgId}&clientName=${encodeURIComponent(formData.clientName)}&fromDate=${formData.fromDate}&toDate=${formData.toDate}&branchName=${formData.branch}`
          );
        } else {
          response = await apiCalls(
            'get',
            `/activities/getScheduleReport?finYear=${finYear}&orgId=${orgId}&clientName=${encodeURIComponent(formData.clientName)}&branchName=${formData.branch}`
          );
        }
        if (response.status === true) {
          console.log('Response:', response);
          setRowData(response.paramObjectsMap.scheduleReportDeatils);
          setIsLoading(false);
          setListView(true);
        } else {
  ...
  const handleDocClick = async (docId, screenCode) => {
    setModalOpen(true);
    try {
      const response = await apiCalls('get', `/activities/getScheduleByDocIdandScreenCode?docId=${docId}&ScreenCode=${screenCode}`);
      if (response.status === true) {
        setFillGridData(response.paramObjectsMap.scheduleVO);
      } else {
  ...
  const handleDownloadPdf = ({ logo, columns, data, fileName, loginUserName, formData }) => {
    ...
    // 4) FILTER METADATA
    const { fromDate, toDate, branch, clientName } = formData;
    doc.setFontSize(9);
    ...
    doc.text('Client Name', xPos + 34, 40);
    ...
    const columnStyles = {
      ...
      8: { cellWidth: 20 }, // assn name
      9: { cellWidth: 40 }, // assn to
  ...
  const handleDownloadExcel = async ({ logo }) => {
    try {
      ...
      const sheet = workbook.addWorksheet('Schedule Report');
      sheet.state = 'visible';
      ...
      titleCell.value = 'Schedule Report';
      ...
      saveAs(blob, `Schedule_Report_${dayjs().format('YYYYMMDD_HHmmss')}.xlsx`);
    } catch (error) {
      ...
      <div className="card w-full p-6 bg-base-100 shadow-xl" style={{ padding: '20px', borderRadius: '10px' }}>
        <>
          <div className="row">
            ...
            {(selectedSections.date || selectedSections.clientName || selectedSections.branch) && (
        ...
        <>
          ...
          >
            ...
            <DialogContent>
              {fillGridData && <>{isLoading ? <FullScreenLoader open={true} /> : <Schedule selectedRow={fillGridData} />}</>}
            </DialogContent>
          </Dialog>
        </>
      </div>
