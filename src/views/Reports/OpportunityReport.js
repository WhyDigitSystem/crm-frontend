...
import Opportunity from 'views/Transaction/Opportunity';
...
function OpportunityReport() {
  ...
  const reportColumns = [
    ...
    { accessorKey: 'mobileNo', header: 'Mobile No', size: 80 },
      ...
      try {
        ...
        if (selectedSections.date) {
          response = await apiCalls(
            'get',
            `/transaction/getOpportunityReport?finYear=${finYear}&orgId=${orgId}&clientName=${encodeURIComponent(formData.clientName)}&fromDate=${formData.fromDate}&toDate=${formData.toDate}&branchName=${formData.branch}`
          );
        } else {
          response = await apiCalls(
            'get',
            `/transaction/getOpportunityReport?finYear=${finYear}&orgId=${orgId}&clientName=${encodeURIComponent(formData.clientName)}&branchName=${formData.branch}`
          );
        }
        if (response.status === true) {
          console.log('Response:', response);
          setRowData(response.paramObjectsMap.opportunityReportDeatils);
          setIsLoading(false);
          setListView(true);
        } else {
  ...
  const handleDocClick = async (docId, screenCode) => {
    setModalOpen(true);
    try {
      const response = await apiCalls('get', `/transaction/getOpportunityByDocIdandScreenCode?docId=${docId}&ScreenCode=${screenCode}`);
      if (response.status === true) {
        setFillGridData(response.paramObjectsMap.opportunityVO);
      } else {
  ...
  const handleDownloadPdf = ({ logo, columns, data, fileName, loginUserName, formData }) => {
    ...
    const columnStyles = {
      ...
      3: { cellWidth: 20 }, // Pro
      4: { cellWidth: 15 }, // Task Name
      5: { cellWidth: 20 }, // Start time
      6: { cellWidth: 40 }, // End Time
      7: { cellWidth: 30 }, // Client name
  ...
  const handleDownloadExcel = async ({ logo }) => {
    try {
      ...
      const sheet = workbook.addWorksheet('Opportunity Report');
      sheet.state = 'visible';
      ...
      titleCell.value = 'Opportunity Report';
      ...
      saveAs(blob, `Opportunity_Report_${dayjs().format('YYYYMMDD_HHmmss')}.xlsx`);
    } catch (error) {
      ...
      <div className="card w-full p-6 bg-base-100 shadow-xl" style={{ padding: '20px', borderRadius: '10px' }}>
        ...
        <>
          ...
          >
            ...
            <DialogContent>
              {fillGridData && <>{isLoading ? <FullScreenLoader open={true} /> : <Opportunity selectedRow={fillGridData} />}</>}
            </DialogContent>
          </Dialog>
        </>
      </div>
