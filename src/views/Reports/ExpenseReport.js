  ...
  const handleDocClick = async (docId, screenCode) => {
    ...
    try {
      const response = await apiCalls(
        'get',
        `/expensedetails/getExpenseDetailsByDocIdandScreenCode?docId=${docId}&ScreenCode=${screenCode}`
      );
      if (response.status === true) {
      ...
      <div className="card w-full p-6 bg-base-100 shadow-xl" style={{ padding: '20px', borderRadius: '10px' }}>
        ...
        <>
          ...
          >
            ...
            <DialogContent>
              {fillGridData && <>{isLoading ? <FullScreenLoader open={true} /> : <ExpenseClaims selectedRow={fillGridData} />}</>}
            </DialogContent>
          </Dialog>
        </>
      </div>
