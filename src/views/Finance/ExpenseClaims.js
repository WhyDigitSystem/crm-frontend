...
export const ExpenseClaims = ({ selectedRow }) => {
  ...
  const getAllExpenseClaims = async () => {
    try {
      const result = await apiCalls(
        'get',
        `/expensedetails/getAllExpenseDetailsByOrgId?branchCode=${branchCode}&createdBy=${employeeCode}&finYear=${finYear}&orgId=${orgId}`
      );
  ...
  const handleSave = async () => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      const safeDate = (dateValue) => {
        return dateValue && dayjs(dateValue).isValid() ? dayjs(dateValue).format('YYYY-MM-DD') : null;
      };
      ...
      const saveFormData = {
        ...
        employeeName: employeeName,
        employeeCode: employeeCode,
        distanceTraveled: formData.distanceTraveled ? parseFloat(formData.distanceTraveled) : 0,
  ...
  const getKPIDetails = async () => {
    try {
      const response = await apiCalls(
        'get',
        `/expensedetails/getExpenseDetailsCount?branchCode=${branchCode}&createdBy=${loginUserName}&finYear=${finYear}&orgId=${orgId}`
      );
      if (response.status === true) {
        ...
        {listView && !isLoading ? (
          <>
            ...
            <Grid container spacing={1} sx={{ mt: 2 }}>
              <Grid item xs={12}>
                <CommonListViewTable data={listViewData} columns={listViewColumns} enableEditing toEdit={getExpenseClaimsById} />
              </Grid>
            </Grid>
          </>
        ) : (
            ...
            <>
              <Grid container spacing={2} mb={3}>
                <Grid item xs={12} sm={3}>
                  <TextField label="Doc ID" variant="outlined" size="small" fullWidth disabled name="docId" value={docId} />
                </Grid>
                <Grid item xs={12} sm={3}>
                  <DatePicker
                    label="Doc Date"
                ...
                <Grid item xs={12} sm={3}>
                  <Autocomplete
                    ...
                    renderInput={(params) => <TextField {...params} label={<span>Expense Type</span>} size="small" fullWidth />}
                  />
                </Grid>
                ...
                <Grid item xs={12} sm={3}>
                  <Autocomplete
                    ...
                    getOptionLabel={(option) =>
                      option?.currency && option?.country ? `${option.currency} - ${option.country}` : option?.currency || ''
                    }
                    value={currencyList.find((item) => item.currency === formData.currency) || null}
                    onChange={(event, newValue) => {
                      handleInputChange({
                        target: {
                          name: 'currency',
                ...
                <Grid item xs={12} sm={3}>
                  <Autocomplete
                    ...
                    // disabled={isLoading || (selectedExpense.approveStatus === 'Approved' || selectedExpense.approveStatus === 'Rejected')}
                    isOptionEqualToValue={(option, value) => option.listOfValues === value.listOfValues}
                    renderInput={(params) => <TextField {...params} label={<span>Payment Mode</span>} size="small" fullWidth />}
                  />
                </Grid>
                <Grid item xs={12} sm={3}>
                  <Autocomplete
