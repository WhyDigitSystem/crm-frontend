...
export const Quotation = ({ selectedRow }) => {
  ...
  const getOpportunityName = async (clientBranch, clientName) => {
    try {
      const response = await apiCalls(
        'get',
        `/transaction/getOpportunityNameIdAndDetails?branchName=${clientBranch}&clientName=${encodeURIComponent(clientName)}&orgId=${orgId}`
      );
      if (response.status === true) {
  ...
  const getProductName = async (oppurtunityId, clientName) => {
    try {
      const response = await apiCalls(
        'get',
        `/transaction/getProductNameFromOpportunity?clientName=${encodeURIComponent(clientName)}&oppurtunityId=${oppurtunityId}&orgId=${orgId}`
      );
      if (response.status === true) {
  ...
  //
  const handlePdfPreview = async (row) => {
    try {
      const res = await apiCalls('get', `/transaction/getAllQuotationById?id=${row.original.id}`);

      if (res.status === true) {
        setPdfRowData(res.paramObjectsMap.quotationVO);
        setPdfOpen(true);
      }
    } catch (err) {
      console.error(err);
    ...
    html2pdf()
      ...
      .then((pdf) => {
        ...
        for (let i = 1; i <= totalPages; i++) {
          ...
          // LEFT
          pdf.text(`Generated On : ${generatedOn}`, 10, pageHeight - 8);

          // CENTER (PAGE NUMBER)
          pdf.text(`Page ${i} of ${totalPages}`, pageWidth / 2, pageHeight - 8, { align: 'center' });

          // RIGHT
          pdf.text(`Generated By : ${loginUserName}`, pageWidth - 10, pageHeight - 8, { align: 'right' });
        }
      })
      .save();
  };
            ...
            <div className="row">
              ...
              <div className="col-md-3 mb-3">
                <Autocomplete
                  ...
                  renderInput={(params) => <TextField {...params} label={<span>Remarks</span>} size="small" fullWidth />}
                />
              </div>
                                    ...
                                    <tr key={row.id}>
                                      ...
                                      <td>
                                        <TextField
                                          ...
                                          // disabled={quotaionById.status === 'APPROVED' || quotaionById.status === 'REJECTED'}
                                          disabled={quotaionById.status === 'APPROVED'}
        ...
        {pdfOpen && (
          <Dialog
            ...
            PaperProps={{
              ...
              initial: { opacity: 0, rotate: -90, y: -80 },
              animate: { opacity: 1, rotate: 0, y: 0 },
              exit: { opacity: 0, rotate: 90, y: -80 },
              transition: {
                type: 'spring',
                stiffness: 120,
                damping: 16
          ...
          >
            <Box id="pdf-content">
              ...
              >
                <Typography variant="h6" fontWeight={600} letterSpacing={1} sx={{ color: 'white' }}>
                  🧾 Quotation
                </Typography>
              </DialogTitle>

              <DialogContent dividers sx={{ px: 1, py: 1 }}>
                {/* COMPANY HEADER */}
                <Box display="flex" justifyContent="space-between" alignItems="center">
                  <Box display="flex" gap={2}>
                    <img src={`data:image/png;base64,${companyDetails.companyLogo}`} width={85} />
                    <Box>
                      <Typography variant="h5" fontWeight={700}>
                        {companyDetails.companyName}
                      </Typography>
                      <Typography fontSize={12}>{`GSTIN: ${companyDetails.gst}`}</Typography>
                      <Typography fontSize={12}>{companyDetails.address}</Typography>
                    </Box>
                  </Box>
                </Box>
                <Divider sx={{ my: 1 }} />
                ...
                >
                  {/* LEFT */}
                  <Box sx={{ display: 'grid', gridTemplateColumns: '80px 10px auto', rowGap: 0.7 }}>
                    <Typography fontWeight={600}>Client Name</Typography>
                    <Typography>:</Typography>
                    <Typography>{pdfRowData.clientName}</Typography>

                    <Typography fontWeight={600}>Branch</Typography>
                    <Typography>:</Typography>
                    <Typography>{pdfRowData.branchName}</Typography>
                    <Typography fontWeight={600}>Contact</Typography>
                    <Typography>:</Typography>
                    <Typography>{pdfRowData.contactName}</Typography>
                    <Typography fontWeight={600}>Email</Typography>
                    <Typography>:</Typography>
                    <Typography>{pdfRowData.email}</Typography>
                    <Typography fontWeight={600}>Mobile No</Typography>
                    <Typography>:</Typography>
                    <Typography>{pdfRowData.mobileNumber}</Typography>
                    <Typography fontWeight={600}>GST No</Typography>
                    <Typography>:</Typography>
                    <Typography>{pdfRowData.gstNo}</Typography>
                    <Typography fontWeight={600}>Address</Typography>
                    <Typography>:</Typography>
                    <Typography>{pdfRowData.address}</Typography>
                  </Box>

                  {/* RIGHT */}
                  <Box sx={{ display: 'grid', gridTemplateColumns: '90px 10px auto', rowGap: 0.7 }}>
                    <Typography fontWeight={600}>Quotation No</Typography>
                    <Typography>:</Typography>
                    <Typography>{pdfRowData.docId}</Typography>
                    <Typography fontWeight={600}>Date</Typography>
                    <Typography>:</Typography>
                    <Typography>{new Date(pdfRowData.docDate).toLocaleDateString('en-GB')}</Typography>
                    <Typography fontWeight={600}>Opportunity ID</Typography>
                    <Typography>:</Typography>
                    <Typography>{pdfRowData.oppurtunityId}</Typography>
                    <Typography fontWeight={600}>Iteration</Typography>
                    <Typography>:</Typography>
                    <Typography>{pdfRowData.iterations}</Typography>
                  </Box>
                </Box>
                {/* ITEM TABLE */}
                <Box mt={1}>
                  <Box sx={{ border: '1px solid #ddd', borderRadius: 1 }}>
                    <Box
                      sx={{
                        display: 'grid',
                        gridTemplateColumns: '1fr 2fr 2fr 2fr 1fr 1fr 1fr 1fr',
                        // background: '#f3f3f3',
                        background: '#EEF3FF',
                        p: 1,
                        fontWeight: 600,
                    ...
                    {pdfRowData.quotationDetailsVO.map((item, i) => (
                      <Box
                        key={i}
                        sx={{
                          display: 'grid',
                      ...
                      >
                        ...
                        <div>{item.qty}</div>
                        <div>{formatINR(item.sellingPrice)}</div>
                        <div>{formatINR(item.discount)}</div>
                ...
                <Box mt={2} px={2} display="flex" justifyContent="space-between">
                  <Box
                    sx={{
                      display: 'grid',
                      gridTemplateColumns: '130px 10px auto',
                      rowGap: 0.6,
                      fontSize: '12px',
                      '& .MuiTypography-root': {
                        fontSize: '12px',
                  ...
                  >
                    <Typography>Amt in Words</Typography>
                    <span>:</span>
                    <span>₹ {pdfRowData.amountInWords}</span>
                    <Typography>Narration</Typography>
                    <span>:</span>
                    <span>₹ {pdfRowData.narration}</span>
                  ...
                  <Box
                    sx={{
                      display: 'grid',
                      gridTemplateColumns: '75px 10px auto',
                      rowGap: 0.6,
                      fontSize: '12px',
                      '& .MuiTypography-root': {
                        fontSize: '12px',
                  ...
                  >
                    <Typography>Gross Amt</Typography>
                    <span>:</span>
              ...
              > */}
                ...
                {/* RIGHT SIDE */}
                {/* <Typography sx={{ fontSize: '12px', fontWeight: 600 }}>Generated By : {loginUserName}</Typography> */}
                {/* </Box> */}
              </DialogContent>
            </Box>
            ...
            >
              ...
              <Button
                ...
                sx={{
                  ...
                  '&:hover': {
                    background: 'linear-gradient(135deg, #4527A0, #283593)'
                  }
                }}
              >
                Download PDF
