...
const SalesOrder = ({ selectedRow }) => {
  ...
  const getQuotationDetails = async (branchName, clientName) => {
    try {
      const response = await apiCalls(
        'get',
        `/transaction/getQuotationNameIdAndDetails?branchName=${branchName}&clientName=${encodeURIComponent(clientName)}&orgId=${orgId}`
      );
      if (response.status === true) {
                                ...
                                {salesOrderDetails.map((detail, index) => {
                                  ...
                                  return (
                                    <tr key={index}>
                                      ...
                                      <td className="text-center pt-3">{amount.toFixed(2)}</td>
                                    </tr>
                                  );
                                })}
                              </tbody>
